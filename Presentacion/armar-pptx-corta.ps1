# ============================================================================
#  Arma CumpleHN-Resumen-editable.pptx con formas y texto reales.
#
...
. (Join-Path $Raiz 'pptx-laminas-corta.ps1')

$Img   = Join-Path $Raiz 'imagenes'
$Notas = Join-Path $Raiz 'notas-corta.txt'
$Dest  = Join-Path $Raiz 'CumpleHN-Resumen-editable.pptx'
...
try {
    ...
    Build-LaminasCorta $pres $Img

    # PowerPoint deja una diapositiva vacía al crear la presentación.
    while ($pres.Slides.Count -gt 7) { $pres.Slides($pres.Slides.Count).Delete() }
