...
La base `BDCUMPLEHN` existe. Las tablas de contenido: `Roles`, `Usuarios`, `Campanas`, `Cargos`, `Departamentos`, `Categorias`, `EstadosPropuesta`, `NivelesVerificacion`, `Candidatos`, `Propuestas`, `Publicaciones`, `Partidos`, `TiposObjeto`, `Valoraciones`, `Comentarios`. Después se sumaron `Auditoria`, `Modulos`, `ConsultasIA`, `Encuestas`, `EncuestaOpciones`, `EncuestaVotos`, `ConfirmacionesCorreo`, `DominiosCorreo`, `Iniciativas`, y las del módulo de espacios: `Espacios`, `EspacioMiembros`, `Suscripciones`, `Planes`, `SolicitudesEspacio`.
...
Usuarios de prueba, todos con contraseña `CumpleHN2026`: `admin` (Administrador) y uno por candidato (`amolina`, `rnunez`, `cvilleda`, `hpaz`, `gfonseca`, `mcastellanos`). Del módulo de espacios, en la base de desarrollo y no en los scripts: `cich` (contraseña `Comisión2027`, con tilde a propósito) administra el espacio de demostración «Colegio de Ingenieros Civiles» (`/e/colegio-de-ingenieros-civiles`), y `admin_infinity` el espacio «Infinity», creado por Roy.
...
Mismo reparto de responsabilidades que la analítica, con una capa más: **la base calcula y además decide qué puede leerse**, el Web Service transporta y comprueba, el modelo redacta, la página dibuja. El modelo **nunca escribe SQL**. Recibe cinco herramientas y cada una es un procedimiento almacenado con parámetros tipados, así que lo único que conoce de la plataforma es la fila que ese procedimiento le devuelve.

Las herramientas son `consultar_tablero` (los diez `spAnalitica*`), `buscar_propuestas`, `ficha_candidato`, `consultar_iniciativas` (script 27) y `catalogos`. Agregar una capacidad nueva es agregar un procedimiento con su `GRANT`, **nunca una consulta suelta**.
...
**Los procedimientos de los scripts 08 y 12 van con `CREATE OR ALTER`, nunca con `DROP` + `CREATE`.** Un `DROP` se lleva en silencio los `GRANT EXECUTE` que el 13 les concede a `cumplehn_ia`, y el síntoma no apunta a la causa: el asistente responde con buena redacción que «la consulta a la base falló», la bitácora `ConsultasIA` lo registra como respondido, y solo se ve con `EXECUTE AS USER = 'cumplehn_ia'`. Pasó el 14 de septiembre de 2026: se volvió a correr el 12 para que `spIACampana` devolviera el espacio y `consultar_tablero` quedó sin permiso hasta repetir el 13. Si algún script vuelve a recrear un objeto de esa lista con `DROP`, hay que correr el 13 después —pide la variable `-v claveIA=...` aunque el login ya exista, porque sin ella sqlcmd se detiene antes de los `GRANT`.
...
**Iniciativas ciudadanas en el asistente** (`27_ia_iniciativas.sql`, herramienta `consultar_iniciativas`). `spIAIniciativas` devuelve en una ida el total, el reparto por categoría y por departamento, y el listado de la más apoyada a la menos (mismo orden que la portada). Del listado van **solo título, categoría, departamento, apoyos y fecha**:

- **Sin descripción.** Son hasta 1,500 caracteres de texto libre sin moderación previa, donde cabe una instrucción dirigida al modelo. Es la razón por la que `Comentarios` queda fuera. El título también lo escribe la ciudadanía, pero son 120 caracteres y el prompt trata todo lo que entra por herramientas como dato.
- **Sin autora.** Los `DENY` del script 18 sobre `Iniciativas` y `vwIniciativas` se conservan: el procedimiento llega a ellas por encadenamiento de propiedad. Verificado con `EXECUTE AS USER = 'cumplehn_ia'`.
- **Respeta el interruptor `iniciativas`**: con el módulo oculto devuelve los cuatro resultados vacíos. El asistente no debe mostrar lo que el sitio escondió.
- El prompt (regla 4) exige llamarlas propuesta ciudadana, no mezclarlas ni sumarlas con las propuestas de campaña, decir que no tienen verificación ni estado de cumplimiento, y aclarar que los apoyos describen a quienes participaron y no a la población.
- El `GRANT` va en el propio 27, que no pide la clave de `cumplehn_ia` como el 13.

**Reporte de consultas** (`26_reportes_ia.sql`, `ReporteAsistente.aspx`). Quien le preguntó al asistente vuelve a sus consultas desde el enlace «Mis consultas y reportes» del bloque, marca las que quiere y genera un reporte con pregunta, respuesta y fecha, que el navegador imprime o guarda como PDF. No gasta un token ni agrega tablas: todo sale de `ConsultasIA`.

- **Cada persona ve solo lo suyo, y lo decide la base.** `spIAHistorialUsuario` y `spIAReporteConsultas` filtran por `codigoUsuario` adentro. Un código de consulta ajeno en la lista se descarta sin error. Solo entran las respondidas.
- **El PDF lo hace el navegador, no el servidor.** Un generador de PDF en el backend habría interpretado el HTML del modelo con otras reglas que la lista blanca, y serían dos garantías que mantener.
- **La lista blanca vive en un solo archivo, `Scripts/cumplehn-respuesta-ia.js`**, que cargan el tablero y el reporte (`CumpleHN.pintarRespuestaIA`). Antes estaba dentro de `cumplehn-analitica.js`. Copiarla en la página del reporte habría sido la copia que se queda atrás sin que se note. En el reporte la respuesta viaja escapada en el atributo `data-gc-respuesta` y nunca se escribe directo en el marcado.
- Ningún procedimiento del 26 se le concede a `cumplehn_ia`.
...
### Módulo de espacios (el modelo de sostenibilidad)

La plataforma se vende como servicio: un **espacio** es un cliente —colegio profesional, cooperativa, sindicato, asociación— que usa el mismo motor para su propio proceso electoral (la elección de su junta directiva), con su sitio, sus planillas, sus candidaturas, su padrón y su tablero. El sitio público CumpleHN sigue gratuito y neutral, y es también un espacio: el único con `esPlataforma = 1`, para que ninguna consulta trate «sin espacio» como un caso aparte. Se hizo en siete etapas (`19_espacios` a `23_cargos`, más el Web Service, el frontend y la página de oferta). **No está en el FO-GR-013, pero el asesor lo aprobó como extensión** (confirmado por Roy el 23 de septiembre de 2026), y ese día se fusionó a `main` junto con las ramas del asistente. Hay un respaldo previo a la migración en `C:\Program Files\Microsoft SQL Server\MSSQL17.SQLEXPRESS\MSSQL\Backup\BDCUMPLEHN_antes_espacios.bak`.

**Un solo mecanismo de aislamiento.** `Campanas.codigoEspacio` es el anclaje: candidaturas, propuestas, publicaciones y encuestas lo heredan por su campaña. Lo que no cuelga de campaña lo lleva directo: `Partidos`, `Iniciativas`, `Auditoria` y `Cargos`. `Categorias` admite NULL como «compartida» (la taxonomía COFOG) más las propias de cada espacio. Los slugs de campañas, candidaturas y partidos **siguen siendo únicos en toda la plataforma**: `/Campana?c=slug` identifica la ficha sin prefijo y la ficha dice a qué espacio pertenece. Lo que pasó a ser único por espacio es el nombre, porque dos organizaciones pueden tener una «Planilla Azul». `UQ_Campanas_unicaActual` es ahora `(codigoEspacio, esActual)`: una destacada por espacio.

**No hay un cuarto rol.** `Usuarios.codigoEspacio`: en NULL, el Administrador administra la plataforma entera y todos los espacios. Con un valor, administra solo ese espacio (es la cuenta del cliente). Candidato y Ciudadano lo llevan siempre en NULL: la candidatura ya dice su campaña, y el ciudadano tiene cuenta global y participa donde el padrón lo admita. Dos funciones en la base: `fnEsAdministrador(@usuario)` es «administra la plataforma» (la usan módulos, espacios, pagos y solicitudes) y `fnEsAdministradorDe(@usuario, @espacio)` es «administra ese espacio», que usa todo lo demás. **El espacio del objeto se deriva del objeto, nunca se recibe de quien llama**: recibirlo sería dejar que el cliente diga sobre qué espacio tiene permiso. Por eso las firmas de verificación y moderación no cambiaron. `fnEspacioDeObjeto(tipo, codigo)` vive en el 19 y no en el 09 porque referencia `Encuestas` e `Iniciativas`, que en una base nueva no existen todavía cuando corre el 09. El Web Service delega en las dos funciones (`EsAdministradorDe`, `EspacioDeObjeto`) para responder lo mismo que la base por definición y no por coincidencia.

**Los `spAnalitica*` exigen `@codigoEspacio` y NULL no es comodín**: «todos los espacios» es exactamente el cruce de datos de dos clientes que no debe existir. **El asistente de IA solo lee el espacio de la plataforma, y lo fijan los procedimientos del 12, no el backend** — `spIACampana` le devuelve el espacio junto con la campaña, porque el login `cumplehn_ia` tiene `DENY` sobre `Espacios`, `vwEspacios`, `EspacioMiembros`, `Suscripciones` y `SolicitudesEspacio`. En un espacio de cliente el bloque del asistente no se dibuja.

**En el frontend las páginas no pasan el espacio: lo inyecta `ContenidoServicio`.** `Servicios/Espacios.cs` resuelve el espacio de la petición con tres fuentes, en este orden: lo que una ficha fijó con `Espacios.Fijar(slug)`, la ruta `e/{espacio}/...`, y dentro de `Admin/` el espacio que la sesión administra (`Sesion.EspacioSlug`). Sin nada de eso, la plataforma. Las consultas públicas viajan con `Espacios.SlugActual` y las de administración con `Sesion.CodigoEspacio`. Es lo que impide que una página nueva se olvide de acotarse. **Las fichas derivan el espacio del objeto** (los cuatro modelos con ficha traen `EspacioSlug`) y lo declaran con `Fijar` apenas lo cargan y antes de pedir nada más: un enlace compartido sin prefijo se dibuja igual dentro de su espacio, con su marca y su pie, y no como contenido de CumpleHN, que sería el problema de neutralidad. `Espacios.Url("~/Campanas")` da la dirección de listado dentro del espacio, y la plataforma nunca lleva prefijo aunque una ficha haya fijado su slug. Las seis rutas `e/{espacio}[/Campanas|Candidatos|Partidos|Propuestas|Analitica]` van en `RouteConfig` antes de FriendlyUrls, sobre las mismas páginas físicas. Un espacio inexistente o retirado manda a la raíz (`Espacios.ExigirExistente`, en `PaginaDeModulo` y en la portada).

**La plantilla declara de quién es lo que se ve.** En un espacio, `Site.Master` pone el nombre del cliente de marca con «con CumpleHN» debajo, el término de agrupación en el menú (`Espacios.terminoAgrupacion`: Partido, Planilla, Lista), «Espacio administrado por» y CumpleHN como herramienta en el pie, sin registro de candidatura ni asistente. Es lo que sostiene la neutralidad del sitio público frente a contenido que administra otro.

**Administración.** `PaginaAdmin` no cambió: `Admin.Master` muestra el espacio administrado bajo la identidad —un desplegable para la plataforma, el nombre fijo para el cliente— y al cambiar recarga la misma página limpia, porque la página ya corrió su `Page_Load` con el espacio anterior. `PaginaAdminPlataforma` protege lo que solo tiene sentido para la plataforma (`Espacios`, `Solicitudes`, `Modulos`), decidido en `OnPreInit` y en la clase base como el resto. «Ver el sitio público» lleva al sitio del espacio administrado, no a la raíz: la cuenta de un cliente llegaba a CumpleHN en lugar de a su sitio. Los cargos son de cada espacio (`23_cargos`): los seis de la plataforma son suyos y una organización crea los suyos en `Admin/Cargos.aspx`, con baja lógica. El alta de candidatura exige cargo activo del espacio.

**La puerta de participación es una sola y gana dos causas.** `MotivoSinParticipacion` (Web Service) exigía cuenta activa y correo confirmado. Ahora recibe el espacio del objeto —derivado, nunca recibido— y comprueba además, en este orden, que el espacio esté vigente (`fnEspacioVigente`) y que la cuenta esté en el padrón (`fnEsMiembro`) cuando `padronCerrado = 1`. Con espacio en cero no comprueba: es un objeto inexistente que otra comprobación rechaza por su lado.

**El padrón es de correos, no de cuentas** (`20_padron`). La organización carga la lista pegada antes o después de que la gente se registre, y la pertenencia se resuelve al participar cruzando el correo normalizado del padrón con el de la cuenta, con `fnCorreoNormalizado`. No se guarda ningún enlace a `Usuarios`: sería calcular dos veces lo mismo y podría quedar desactualizado. Quitar es baja lógica, lo ya votado se conserva. `vwEspacioMiembros` le dice a la organización cuántos ya tienen cuenta confirmada sin exponer más que el nombre. Lo que el padrón no resuelve, y hay que declarar: dice que un buzón está en la lista, no quién lo controla. El plan vigente pone un tope al padrón (`Suscripciones.maxMiembros`) y `spAdminCargarPadron` lo hace cumplir: un plan que dijera «hasta 250» sin que nada lo cumpliera sería una regla de mentira.

**No hay pasarela de pago, y no hay botón que la simule** (`21_suscripciones`, `22_solicitudes`). La organización pide el espacio desde la página pública `Organizaciones.aspx` (hero, para quién, qué incluye, cómo funciona, planes, cómo se paga, preguntas y formulario con campo trampa), la solicitud queda en `SolicitudesEspacio` y en la bandeja `Admin/Solicitudes.aspx` (con aviso por correo a `CorreoSolicitudes` si está configurado, y la solicitud se guarda igual si el correo falla), la plataforma la convierte en espacio con «Crear el espacio» —que precarga el alta y, al guardar, marca la solicitud atendida y abre el panel de pagos—, cobra por transferencia, y registra el pago con su referencia. El espacio queda vigente por el período del plan. Los tres planes viven en la tabla `Planes` (Proceso L 1,500 / 45 días / 250 miembros, Institución L 3,500 / 90 días / 1,000, Anual L 12,000 / 365 días / 5,000): el precio se cambia ahí, no en la página, y la suscripción copia plan y tope para que lo vendido no cambie si el plan cambia. `EspacioDemo` en el Web.config del frontend elige el espacio de ejemplo de la página. La pasarela local va como trabajo futuro con su costo por transacción, igual que el SMS.

**El estado de un espacio se deriva, no se guarda**: `vwEspacios` responde Plataforma, Vigente, Vencido, Sin suscripción o Retirado, y `Espacio.Estado` en el frontend lo lee tal cual. **Vencido no es retirado**: el vencido se lee (la franja del sitio lo dice) pero no se participa ni se administra —`fnEsAdministradorDe` se redefine en el 21 para exigirle vigencia a la cuenta del cliente, no a la de la plataforma, que es quien registra el pago que lo reactiva. Retirado sale de toda consulta y desactiva sus cuentas. Un espacio recién creado no está vigente hasta su primer pago, y la página lo lleva ahí.

**Cuidados prácticos.** `plan` es palabra reservada en T-SQL: la columna es `nombrePlan`. Regenerar el proxy con SvcUtil deja BOM y CRLF, y el original es LF sin BOM: normalizar antes de confirmar, además de corregir el `ConfigurationName`. En Perl, `\E` dentro de un reemplazo termina el modo de mayúsculas y se come la barra: `Modelos\\Espacios.cs` sale como `Modelosspacios.cs` — los archivos con rutas de Windows se editan con `sed` o desde un archivo. Los `.csproj` y algunos scripts (`12`, `13`) son CRLF: comprobar con `file` antes de insertar líneas.
...
### Archivos: foto de perfil y documentos de respaldo

`25_archivos.sql`. **Los bytes viven en disco y la base guarda solo el nombre.** El recorrido es siempre el mismo y no tiene atajos:

```
subir:  página del panel (FileUpload) → frontend Servicios/Archivos.cs (WebClient)
        → backend SubirArchivo.ashx → disco + spArchivoRegistrar
ver:    <img src="~/Archivo.ashx?id=N"> → frontend Archivo.ashx
        → backend VerArchivo.ashx → spArchivoObtener → disco
```

- **La carpeta es `App_Data/Archivos` del backend**, que IIS nunca sirve directamente: pedirla por dirección da 404. La única manera de leer un archivo es el manejador, que antes busca su fila. Va en el `.gitignore`.
- **El nombre en disco es un GUID generado por el backend**, nunca el que trae la persona, y `Path.GetFileName` impide salir de la carpeta aunque una fila trajera `..\`.
- **El tipo se reconoce por los primeros bytes** (`FF D8 FF`, `89 50 4E 47`, `%PDF`), no por la extensión. Foto: JPG o PNG hasta 2 MB. Respaldo: PDF, JPG o PNG hasta 5 MB, cinco por proyecto. Por eso los dos `Web.config` llevan `maxRequestLength="6144"`.
- **Si la base rechaza, el manejador borra el archivo que acaba de escribir**: no queda nada en disco sin su fila.
- **Nada se borra**: quitar es baja lógica, y `spArchivoObtener` deja de entregarlo.
- **Una sola foto activa**, garantizada por el índice filtrado `UQ_Archivos_unaFoto`. Cambiarla devuelve el perfil a «Declarado» (es presentación). Un respaldo no toca la verificación y se puede adjuntar aunque el proyecto tenga reacciones: respalda lo prometido, no lo cambia.
- Las consultas de candidatura y publicación devuelven `codigoFoto` con `fnFotoCandidato`, y el frontend lo convierte en `~/Archivo.ashx?id=N` (`Archivos.Url`). La columna `Candidatos.fotoUrl` quedó sin uso.
- La dirección del backend para los manejadores es el `appSetting` `BackendUrl` del frontend, por HTTP 51720.
- Para borrar a mano filas de `Archivos` con sqlcmd hace falta `-I`: sin `QUOTED_IDENTIFIER` el índice filtrado da el error 1934.
